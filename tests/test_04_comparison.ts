...
describe('TestOrdering', () => {
  ...
  it('same unit: 500 m < 1000 m', () => {
    expect(ops.compare(lit('500 m'), lit('1000 m'))).to.equal(-1)
  })

  it('cross-unit mass: 500 g < 1 kg', () => {
    expect(ops.compare(lit('500 g'), lit('1 kg'))).to.equal(-1)
  })

  it('cross-unit time: 59 min < 1 h', () => {
    expect(ops.compare(lit('59 min'), lit('1 h'))).to.equal(-1)
  })

  it('cross-unit energy: 1 eV < 1 J', () => {
    expect(ops.compare(lit('1 eV'), lit('1 J'))).to.equal(-1)
  })

  it('mass: 1 kg > 999 g', () => {
    expect(ops.compare(lit('1 kg'), lit('999 g'))).to.equal(1)
  })

  it('speed: 10 m/s > 30 km/h', () => {
    expect(ops.compare(lit('10 m/s'), lit('30 km/h'))).to.equal(1)
  })

  it('acceleration: 9.81 m/s2 > 9.0 m/s2', () => {
    expect(ops.compare(lit('9.81 m/s2'), lit('9.0 m/s2'))).to.equal(1)
  })

  it('equal cross-unit: 1 km == 1000 m returns 0', () => {
    expect(ops.compare(lit('1 km'), lit('1000 m'))).to.equal(0)
  })

  it('equal cross-unit: 1000 m == 1 km returns 0', () => {
    expect(ops.compare(lit('1000 m'), lit('1 km'))).to.equal(0)
  })

  it('incompatible dimensions throws', () => {
    expect(() => ops.compare(lit('1 m'), lit('1 kg'))).to.throw()
  })

  it('incompatible dimensions throws: m vs s', () => {
    expect(() => ops.compare(lit('1 m'), lit('1 s'))).to.throw()
  })
...
describe('TestCompoundAndDerivedComparison', () => {
  ...
  it('derived unit: 2 N > 1 kg.m/s2', () => {
    expect(ops.compare(lit('2 N'), lit('1 kg.m/s2'))).to.equal(1)
  })

  it('compound force: 10 kg.m/s2 > 5 N', () => {
    expect(ops.compare(lit('10 kg.m/s2'), lit('5 N'))).to.equal(1)
  })

  it('compound pressure: 101325 Pa > 1 bar', () => {
    expect(ops.compare(lit('101325 Pa'), lit('1 bar'))).to.equal(1)
  })
  ...
  it('energy: 1 kJ > 1 J', () => {
    expect(ops.compare(lit('1 kJ'), lit('1 J'))).to.equal(1)
  })

    it('compound with astronomical units: 1 [ly].kg/(AU.s2) > 1 m.kg/(AU.s2)', () => {
    expect(ops.compare(lit('1 [ly].kg/(AU.s2)'), lit('1 m.kg/(AU.s2)'))).to.equal(1)
    })

  it('compound unit: 500 g.m/s2 < 1 kg.m/s2', () => {
    expect(ops.compare(lit('500 g.m/s2'), lit('1 kg.m/s2'))).to.equal(-1)
  })
...
describe('TestTemperatureComparison', () => {
  ...
  it('300 K > 273.15 K', () => {
    expect(ops.compare(lit('300 K'), lit('273.15 K'))).to.equal(1)
  })

  it('300 K > 0 Cel', () => {
    expect(ops.compare(lit('300 K'), lit('0 Cel'))).to.equal(1)
  })

  it('200 K < 0 Cel', () => {
    expect(ops.compare(lit('200 K'), lit('0 Cel'))).to.equal(-1)
  })

  it('100 K > 90 K', () => {
    expect(ops.compare(lit('100 K'), lit('90 K'))).to.equal(1)
  })
...
describe('TestLargeValueComparison', () => {
  ...
  it('1E808 m > 1E307 m — Infinity vs finite float', () => {
    expect(ops.compare(lit('1E808 m'), lit('1E307 m'))).to.equal(1)
  })

    it('1E700 km vs 1E703 m — both overflow to Infinity, NaN diff returns 1', () => {
    expect(ops.compare(lit('1E700 km'), lit('1E703 m'))).to.equal(1)
    })

  it('big < bigger', () => {
    const big  = '1' + '0'.repeat(300) + ' m'
    const bigger = '1' + '0'.repeat(300) + '1 m'
    expect(ops.compare(lit(big), lit(bigger))).to.equal(-1)
  })
...
describe('TestSmallValueComparison', () => {
  ...
  it('16 significant digits collapse to same float — returns 0', () => {
    expect(ops.compare(lit('1.000000000000000003 m'), lit('1.000000000000000002 m'))).to.equal(0)
  })
...
describe('TestDimensionlessComparison', () => {
  ...
  it('0.5 1 < 1 1', () => {
    expect(ops.compare(lit('0.5 1'), lit('1 1'))).to.equal(-1)
  })
